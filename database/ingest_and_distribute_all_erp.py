...
def main():
    ...
    print(f"   -> Loaded {len(pmdb_rows)} PMDB summary records.", flush=True)
    ...
    print("[+] Preserving explicit PM assignments & distributing unassigned projects across all PMs...", flush=True)
    ...
    # Fetch projects from npms/docs to distribute across all 18 PMs
    cur.execute("SELECT header_id FROM public.xx_nic_pm_prj_list WHERE prj_mgr_id = 1826 ORDER BY header_id;")
    sherpa_projects = [r[0] for r in cur.fetchall()]

    for idx, hid in enumerate(sherpa_projects):
